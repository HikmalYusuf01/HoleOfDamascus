...
class Artikel(models.Model):
    ...
    isi = RichTextUploadingField(
        ...
        external_plugin_resources=[(
            'youtube',
            'http://http://hikmal.my-kelompok2.my.id//static/ckeditor_plugins/youtube/youtube/',
            'plugin.js',
        )],
        blank=True,
        null=True
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            self.slug = slugify(f"{current_time.year}-{current_time.month}-{current_time.day}-{self.judul}")
